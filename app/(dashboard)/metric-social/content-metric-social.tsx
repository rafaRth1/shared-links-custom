...
import { AreaChart, CardRedSocial, DoughnutChart } from "@/components";
import { clientAxios } from "@/utils";
import { MetricSocial } from "@/types/metric-social";
...
export const ContentMetricSocial = ({ session }: Props) => {
  const [metric, setMetric] = useState({} as MetricSocial);
  ...
  useEffect(() => {
    const getMetricsProfile = async () => {
      const response = await clientAxios.post("/bio/profile-metric", {
        ...
        start: new Date(valueDate.end.toString()).toISOString(),
        end: new Date(valueDate.start.toString()).toISOString(),
      });
  ...
  const dateView = metric.profileMetric?.totalViews.list.map(
    (view) => view.key
  );

  const valueView = metric.profileMetric?.totalViews.list.map(
    (view) => view.value
  );
    ...
    <section className="metric-social">
      <div className="flex justify-between flex-wrap mb-5">
        ...
        <DateRangePicker
          ...
          className="max-w-xs text-neutral-200"
          classNames={{
            inputWrapper: "bg-[#2D2C2D]",
          }}
        />
      </div>
      ...
      <div>
        <div className="flex w-full mb-5">
          <CardRedSocial
            ...
            variants="max-w-[25%] basis-1/4 pr-5"
          />
          ...
          <CardRedSocial
            ...
            sizeIcon={20}
            nameCount="220 mes pasado"
            variants="max-w-[25%] basis-1/4 "
          />
        </div>

        <div className="flex mb-5">
          <div className="max-w-[75%] basis-3/4 pr-5">
            <AreaChart
              variants="flex-1"
              name="Seguidores"
              labels={dateView}
          ...
          <div className="max-w-[25%] basis-1/4 bg-neutral-950"></div>
        </div>
