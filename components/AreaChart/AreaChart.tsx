...
export function AreaChart({ variants }: { variants?: string }) {
	...
	useEffect(() => {
		const handlerTypeDate = async () => {
			if (typeof window !== 'undefined' && window.localStorage) {
				...
				if (access_token) {
					try {
						const response = await getFollowersTypeDate(id, typeDate, access_token);
						...
						const newArrayLabels = response[0].values.map(
							(value: { value: string; end_time: string }) => value.end_time.split('-')[2].split('T')[0]
						);
						setValuesChart({
	...
	return (
		<div className={`card-area-chart bg-[#2D2C2D] border border-neutral-600 rounded-md p-5 ${variants}`}>
