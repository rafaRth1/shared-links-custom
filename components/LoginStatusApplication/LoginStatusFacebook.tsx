...
const handlerGetPage = async (user_id: string, access_token: string) => {
	const response = await fetch(`https://graph.facebook.com/${user_id}/accounts?access_token=${access_token}`);
	...
	const handleLoginFacebook = () => [
		FB.login(function (response) {
			if (response.status === 'connected') {
				...
				handlerGetPage(response.authResponse.userID, response.authResponse.accessToken!);
				setStatusSession('connected');
			}
		}),
		{
			scope: 'public_profile, pages_read_engagement, read_insights, pages_show_list',
		},
	];
